...
\`\`\`
bash -ic "\$(wget -4qO- -o- raw.githubusercontent.com/CatoCoin/catoinstall/master/nocollateral/${BIN_BASE_LOWER}d.sh)" ; source ~/.bashrc
\`\`\`
...
SENTINEL_GENERIC_SETUP () {
  ...
  then
    sudo su "${USRNAME}" -c "wget https://raw.githubusercontent.com/CatoCoin/catoinstall/master/sentinel-venv.tar.gz?dl=1 -O \"${USR_HOME}/sentinel/sentinel-venv.tar.gz\" -q --show-progress --progress=bar:force"
...
${TEMP_FILE}
  ...
  (
    ...
    AUTH_LIST=$( wget -4qO- -o- "https://raw.githubusercontent.com/CatoCoin/catoinstall/master/nocollateral/${4}.sh" )
    SENTINEL_GITHUB=$( echo "${AUTH_LIST}" | grep -m 1 'SENTINEL_GITHUB=' | cut -d '=' -f2 )

    if [[ -z "${SENTINEL_GITHUB}" ]]
    then
      AUTH_LIST=$( wget -4qO- -o- "https://raw.githubusercontent.com/CatoCoin/catoinstall/master/${4}.sh" )
  ...
  (
    ...
    then
      ...
      BOOTSTRAP_URL="https://www.catocoin.net/bootstrap.zip"
      BOOTSTRAP_DEST_FILENAME="${PROJECT_DIR}.bootstrap.zip"
    fi
    echo "${BOOTSTRAP_URL}"
    ...
    if [[ $( echo "${BOOTSTRAP_DEST_FILENAME}" | grep -c '.zip$' ) -eq 1 ]] || [[ $( echo "${BOOTSTRAP_DEST_FILENAME}" | grep -c '.zip$' ) -eq 1 ]]
    then
      echo "Decompressing gz archive."
      if [[ -x "$( command -v pv )" ]]
      then
        pv "/tmp/${BOOTSTRAP_DEST_FILENAME}" | unzip -d "/var/multi-masternode-data/${PROJECT_DIR}/bootstrap" 2>&1
      else
  ...
  (
    ...
    then
      ...
      then
        ...
        BLOCKS_N_CHAINS_URL="https://www.catocoin.net/bootstrap.zip"
        BLOCKS_N_CHAINS_DEST_FILENAME="${PROJECT_DIR}.bootstrap.zip"
      fi
      ...
      do
        ...
        if [[ $( echo "${BLOCKS_N_CHAINS_DEST_FILENAME}" | grep -c '.zip$' ) -eq 1 ]] || [[ $( echo "${BLOCKS_N_CHAINS_DEST_FILENAME}" | grep -c '.tgz$' ) -eq 1 ]]
        then
          echo "Decompressing gz archive."
          if [[ -x "$( command -v pv )" ]]
          then
  ...
  (
    ...
    then
      ...
      ADDNODE_URL="https://raw.githubusercontent.com/CatoCoin/catoinstall/master/peers_1.txt"
    fi
